	...
	<div class="lb_wrapper">
		...
		<div class="form">
			<form action="<?php echo (isset($action) ? $action : ''); ?>" method="POST" name="idcf_level_select">
				<div class="form-row inline left ">
				...
				<div class="form-row inline  total">
					<label for="total">Enter your Donation Amount<?//php _e('Total', 'ignitiondeck'); ?></label>
					<?php if (isset($pwyw) && $pwyw) { ?>
						<input type="text" class="total" name="total" id="total" value="<?php echo "Enter donation amount";// echo total; ?>" />
					<?php } else { ?>
